<?php
  include '../koneksi.php';
?>
<!DOCTYPE html>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
<title>Kantorku Inventory</title>
<link href="../karre.css" rel="stylesheet" />

</head>

<body>
<div class="nav">
    <div class="bingkai">
        <div class="menu">
        <ul>
                <li><a href="../beranda.php">Beranda</a></li>
                <li><a href="../admin/admin.php">Operator Gudang</a></li>
                <li><a href="../data/barang.php">Barang</a></li>
                <li><a href="#">Data Barang</a>
                  <ul>
                <li><a href="brg_masuk.php">Barang Masuk</a></li>
                <li><a href="brg_keluar.php">Barang Keluar</a></li>
                
                  </ul>
                </li>
                <li><a href="#">Laporan</a>
                  <ul>
                    <li><a href="../cetak/pemasukan.php" target="tengah">Pemasukan</a></li>
                    <li><a href="../cetak/pengeluaran.php" target="tengah">Pengeluaran</a></li>
                  </ul>
                </li>  
                <li><a href="#">Search Data</a>
                  <ul>
                    <li><a href="cari/brg_masuk.php" target="tengah">Barang Masuk</a></li>
                    <li><a href="cari/brg_keluar.php" target="tengah">Barang Keluar</a></li>
                  </ul>
                </li>       
                <li><a href="../index.php">Logout</a></li>
            </ul>
        </div>
    </div>  
</div>
<div class="jarak_atas bingkai">
<div class="judul">Detail Pemasukan Barang</div>
<form method="post" action="brg_masuk.php" enctype="multipart/form-data">
<center><input type="submit" value="KEMBALI" class="tombol" required/></center>
  <table width="1000px" border="0">
      <tr align="center">
        <td width="40" height="49"><strong>No</strong></td>
        <td width="148"><strong>Tanggal</strong></td>
        <td width="148"><strong>Barcode</strong></td>
        <td width="148"><strong>Merk Barang</strong></td>
        <td width="148"><strong>Nama Barang</strong></td>
        <td width="148"><strong>Tanggal Produksi</strong></td>
        <td width="148"><strong>Pemasukan</strong></td>
        <td width="148"><strong>Status</strong></td>
      </tr>
      <?php
      $no=1;
      $id_pemasukan=$_GET['id_pemasukan'];
      $sql="SELECT barang.barcode, barang.merk_barang, barang.nama_barang, pemasukan.*, det_pemasukan.* from ((barang inner join pemasukan on barang.id_barang=pemasukan.id_barang) inner join det_pemasukan on pemasukan.id_pemasukan=det_pemasukan.id_pemasukan) WHERE det_pemasukan.id_pemasukan = '$id_pemasukan'";
      $query= mysqli_query($db_link,$sql);
      while($data = mysqli_fetch_array($query))
        {
        ?>
        <tr align="center">
          <td width="40"><?php echo "$no"; ?></td>
          <td width="148"><?php echo "$data[tanggal]"; ?></td>
          <td width="148"><?php echo "$data[barcode]"; ?></td>
          <td width="148"><?php echo "$data[merk_barang]"; ?></td>
          <td width="148"><?php echo "$data[nama_barang]"; ?></td>
          <td width="148"><?php echo "$data[tgl_produksi]"; ?></td>
          <td width="207"><?php echo "$data[pemasukan]"; ?></td>
          <td width="148"><?php echo "$data[status]"; ?></td>
          </td>
        </tr>
        <?php
        $no++;
        }
    ?>
    </table>

</div>
</body>
</html>